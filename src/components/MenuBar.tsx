...
import { getAllFiles } from '../lib/history-manager';
...
const MenuBar: React.FC<MenuBarProps> = ({
        ...
        submenu:
          recentFiles.length > 0
            ? recentFiles.slice(0, 10).map((f) => ({
                ...
                action: `open-candidate:${f.path}`,
                disabled: f.is_alive === 0,
              }))
            : [{ label: '履歴がありません', action: '' }],
      },
      {
        label: '形式を選択して開きなおす',
        submenu: [
    ...
    表示: [
      {
        ...
        checked: checkedItems['view:line-numbers'] !== false,
        action: 'set-line-numbers',
      },
      {
        label: 'ミニマップを表示',
        type: 'checkbox',
        id: 'view:minimap',
        checked: checkedItems['view:minimap'] === true,
        action: 'set-minimap',
      },
      {
        label: 'アウトラインを表示',
        type: 'checkbox',
        id: 'view:outline',
        checked: checkedItems['view:outline'] !== false,
        action: 'set-show-outline',
      },
      {
        label: 'ルビ・傍点ツール',
        type: 'checkbox',
        id: 'view:ruby-toolbar',
        checked: checkedItems['view:ruby-toolbar'] !== false,
        action: 'set-show-ruby-toolbar',
      },
      {
        label: 'Markdownツール',
        type: 'checkbox',
        id: 'view:markdown-toolbar',
        checked: checkedItems['view:markdown-toolbar'] !== false,
        action: 'set-show-markdown-toolbar',
      },
      {
        label: '折り返し',
        type: 'checkbox',
        id: 'view:word-wrap',
        checked: checkedItems['view:word-wrap'] !== false,
      ...
      },
      {
        ...
        type: 'checkbox',
        id: 'view:preview',
        checked: checkedItems['view:preview'] === true,
        action: 'set-preview',
      },
    ],
    設定: [
        ...
        submenu: [
          {
            ...
            type: 'radio',
            id: 'theme:vs-dark',
            checked: checkedItems['theme:vs-dark'] === true,
          ...
          },
          {
            ...
            type: 'radio',
            id: 'theme:light',
            checked: checkedItems['theme:light'] === true,
          ...
          },
          {
            ...
            checked: checkedItems['theme:organic-note'] === true,
            action: 'set-theme:organic-note',
          },
          {
            label: '月光',
            type: 'radio',
            id: 'theme:moonlight',
            checked: checkedItems['theme:moonlight'] === true,
            action: 'set-theme:moonlight',
          },
          {
            label: '長良川',
            type: 'radio',
            id: 'theme:nagaragawa',
            checked: checkedItems['theme:nagaragawa'] === true,
            action: 'set-theme:nagaragawa',
          },
          {
            label: '刀',
            type: 'radio',
            id: 'theme:katana',
            checked: checkedItems['theme:katana'] === true,
            action: 'set-theme:katana',
          },
        ],
      },
      {
  ...
  // ウィンドウの最大化状態は useTauriWindow が管理するため不要
  ...
  useEffect(() => {
    ...
    getAllFiles()
      .then((files) => {
        // HMFileRecord（file_path）をMenuBar用の形式（path）にマップ
        ...
        const sorted = mapped.slice().sort((a, b) => (b.id || 0) - (a.id || 0));
